...
*
* $Proj:
* RecaserAndDetokenLib for Statistical Machine Translation
*
* $Func:
* interface
*
...
bool PrintRecaserAndDetokenLogo();


/*
* $Name: __Init
* $Funtion: Init for RecaserAndDetoken
* $Date: 2014-10-10
*/
void*  __init(const char* config) {
  PrintRecaserAndDetokenLogo();
  cerr<<"Parameters:\n"<<flush;
  cerr<<"  config_file_      :  "<<config<<"\n"<<flush;
  map< string, string > parameters_for_config;
  ParseParameterInConfig( config, parameters_for_config );

  RecasingAndDetokenInterface * interf = new RecasingAndDetokenInterface();
  ...
  interf->config_recasing_and_detoken_.recase_flag_     = true;
  interf->config_recasing_and_detoken_.support_service_flag_ = true;
  interf->config_recasing_and_detoken_.Init(parameters_recaser);
  interf->config_recasing_and_detoken_.nbest_ = 1;
  cerr<<"Initialize Recasing Model...\n"<<flush;
  interf->model_recasing_.Init(interf->config_recasing_and_detoken_);
  return ( void* ) interf;
}
...
/*
* $Name: __reload
* $Funtion: reload model for RecaserAndDetoken
* $Date: 2014-01-12
*/
...
/*
* $Name: __do_job
* $Funtion: do job for RecaserAndDetoken
* $Date: 2014-01-12
*/
char* __do_job( void* class_handle, const char* msg_text, int print_log, const char* log_head ) {
  ...
  cerr<<"Recasing...";
  RecasingAndDetokenInterface * interf = (RecasingAndDetokenInterface *) class_handle;
  string sentence(msg_text);
  string recasing_translation_result;
  string final_translation_result;

  SourceSentence source_sentence_recasing;
  source_sentence_recasing.Init(sentence);
  Recasing recasing;
//  recasing.Decode(interf->config_recasing_and_detoken_, interf->model_recasing_, source_sentence_recasing, final_translation_result);
  recasing.Decode(interf->config_recasing_and_detoken_, interf->model_recasing_, source_sentence_recasing, recasing_translation_result);
  Detokenizer detokenizer_handle;
  detokenizer_handle.DetokenizerEn(recasing_translation_result, final_translation_result);
  ...
  cerr<<"Done!\n"
      <<"[INPUT  ]  "<<sentence<<"\n"
      <<"[RECASER]  "<<recasing_translation_result<<"\n"
...
/*
* $Name: __destroy
* $Funtion: destroy model for RecaserAndDetoken
* $Date: 2014-01-12
*/
